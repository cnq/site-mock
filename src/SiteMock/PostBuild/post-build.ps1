...
& "$THIS_SCRIPTS_DIRECTORY\New-NuGetPackage.ps1" -NuGetExecutableFilePath "$THIS_SCRIPTS_DIRECTORY\..\..\.nuget\NuGet.exe" -ProjectFilePath "$ProjectFilePath" -VersionNumber $versionNumber -ReleaseNotes $releaseNotes -PackOptions "-OutputDirectory ""$OutputDirectory"" -Properties $packProperties -NonInteractive $additionalPackOptions" -DoNotUpdateNuSpecFile -NoPrompt -Verbose